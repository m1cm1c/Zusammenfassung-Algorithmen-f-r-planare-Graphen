...
\begin{enumerate}
    ...
    \item Konstruiere zu $G^*$ einen Graphen $G' = (V', E')$, so dass ein perfektes Matching minimalen Gewichts $M$ in $G'$ eine \gls{geradeMenge} maximalen Gewichts $M^*$ in $G^*$ induziert. Berechne $M$ und entspr. Menge $M^*$. Falls $M^* \neq \emptyset$, gib duale Menge zu $M^*$ aus.
